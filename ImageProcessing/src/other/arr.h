...
namespace std
{
    namespace oth
    {
        ...
        public:
            ...
            {
                if (curr._size != other._size || curr.num != other.num)
                {
                    return false;
                }
                bool checker = true;
            ...
            friend std::ostream &operator<<(std::ostream &output, const arr &other) noexcept
            {
                ...
                {
                    if (other.check[i] == true)
                    {
                        output << other._arr[i] << std::endl;
                    }
                }
                return output;
            }
            ...
            {
                if (other.num + 1 >= other._size)
                {
                    other.resize(other._size + 1);
                }
                other.check[other.num] = true;
                input >> other._arr[other.num++];
                return input;
            }
            ...
            inline void resize(size_t new_size)
            {
                if (this->_size != new_size)
                {
                    ...
                    bool *c_tmp = new bool[new_size];
                    size_t comp = new_size < this->_size ? new_size : this->_size;
                    for (int i = 0; i < new_size; i++)
                    {
                        tmp[i] = i < comp ? this->_arr[i] : TYPE();
                        c_tmp[i] = i < comp ? this->check[i] : false;
                    }
                    this->_size = new_size;
                    this->num = this->num > this->_size ? this->_size : this->num;
                    delete[] this->_arr;
                    this->_arr = new TYPE[new_size];
                    this->_arr = tmp;
                    delete[] this->check;
                    this->check = new bool[new_size];
                    this->check = c_tmp;
                }
            }
            ...
            inline void compact()
            {
                TYPE *tmp = new TYPE[this->num + 1];
                bool *c_tmp = new bool[this->num + 1];
                int new_size = 0;
            ...
            inline void push_back(TYPE element)
            {
                ...
                {
                    if (b_pos + 1 >= this->_size)
                    {
                        resize(this->_size + 1);
                    }
                    this->check[b_pos + 1] = true;
                    this->_arr[b_pos + 1] = element;
                    ++this->num;
                }
            }
            ...
            inline void push_front(TYPE element)
            {
                ...
                bool *c_tmp = new bool[this->_size];
                c_tmp[f_pos] = true;
                tmp[f_pos] = element;
                for (size_t i = f_pos; i < this->_size - 1; i++)
                {
                    c_tmp[i + 1] = this->check[i];
                    tmp[i + 1] = this->_arr[i];
                }
                ++this->num;
                delete[] this->_arr;
            ...
            {
                std::oth::arr<TYPE> _unique;
                _unique.set(0, this->_arr[0]);
                int cnt = 0;
                ...
                for (size_t i = 0; i < this->size(); i++)
                {
                    ...
                    {
                        if (this->_arr[i] == _unique[j])
                        {
                            checker = true;
                            break;
                        }
                    }
                    if (checker == false && this->check[i] == true)
                    {
                        _unique.set(cnt++, this->_arr[i]);
                    }
                }
